...
<html>
  ...
  <body>
  <h3>这是收文管理页面</h3>
    <form action="receipt/edit" method="post" name="Form" id="Form">
    	...
    	<script id="container" name="content" type="text/plain" >
		   ...
		   分发：<input id="DISTRIBUTE" name="DISTRIBUTE" type="text" readonly onclick="showmenu();" value="${DISTRIBUTE}"/>
		   <input id="DISTRIBUTE_ID" name="DISTRIBUTE_ID" type="hidden"  value="${DISTRIBUTE_ID}"/>
		   传阅：<input id="TRANSFER" name="TRANSFER" type="text" readonly onclick="showmenu2();"/>
  ...
  <script>
    ...
    function disagree(){
  	...
  	function cancel(){
  		location.href="<%=basePath%>receipt/list";
  	}
  	var dispatchId="<%=request.getParameter("DISPATCH_ID")%>";
  	//树结构搭建
  	var setting = {
	   		async : {
				enable : true,
				type : 'post',
				otherParam:["dispatchId",dispatchId],
				url : "receipt/tree"
			},
	   		check: {
				enable: true,
