...
<html>
				...
				<input type="hidden" name="id" value="${var.dispatch_id}">
							<tr>
								<td  style="width: 30px;">${vs.index+1}</td>
										...
										<td><c:choose>
										<c:when test="${var.status=='0'}">未校核</c:when>
										<c:when test="${var.status=='1'}">已校核，等待审批中</c:when>
										<c:when test="${var.status=='2'}">审校核未通过</c:when>
										<c:when test="${var.status=='3'}">审批通过</c:when>
										<c:when test="${var.status=='4'}">审批未通过</c:when>
										</c:choose></td>
  ...
  <table width="100%" border="0" align="center" cellpadding="0"
										...
										<tr>
											...
											<td width="60%" align="right">
												[
												<a href="approver/list?page=1"
													class="right-font08">首页</a> |
												<a
													href="approver/list?page=${page.pagebefore}"
													class="right-font08">上一页</a> |
												<a
													href="approver/list?page=${page.pageafter}"
  ...
  <script>
  	...
  	function del(Id){
  			...
  			if(confirm("确认删除吗？")){
  				location.href="<%=basePath%>check/del?DISPATCH_ID="+Id;
  			}
  		
  	}
  	function edit(Id){
  		location.href="<%=basePath%>approver/goedit?DISPATCH_ID="+Id;
  	}
  </script>
</html>
