...
<html>
				...
				<input type="hidden" name="id" value="${var.dispatch_id}">
							<tr>
								<td  style="width: 30px;">${vs.index+1}</td>
										...
										<td><input type="button" value="修改" onclick="edit('${var.dispatch_id}');"></td>
  ...
  <script>
  	...
  	function del(Id){
  			...
  			if(confirm("确认删除吗？")){
  				location.href="<%=basePath%>dispatch/del?DISPATCH_ID="+Id;
  			}
  		
  	}
  	function edit(Id){
  		location.href="<%=basePath%>dispatch/goedit?DISPATCH_ID="+Id;
  	}
  </script>
</html>
