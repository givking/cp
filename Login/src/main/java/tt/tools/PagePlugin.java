			...
			if(mappedStatement.getId().matches(pageSqlId)){ //拦截需要分页的SQL
				...
				if(parameterObject==null){
					...
					if(parameterObject instanceof PageUtil){	//参数就是Page实体
						 ...
						 page.setPagelistcount(6);//设置每页数据显示条数
						page.setListcount(count);
						page.getPagecount();
						page.getPageafter();
						page.getPagebefore();
					}else{	//参数为某个实体，该实体拥有PageUtil属性
	...
	private String generatePageSql(String sql,PageUtil page){
		if(page!=null && dialect!=null){
			...
			if("mysql".equals(dialect)){
				pageSql.append(sql);
				pageSql.append(" limit "+page.getStartindex()+","+page.getPagelistcount());
			}else if("oracle".equals(dialect)){
				pageSql.append("select * from (select tmp_tb.*,ROWNUM row_id from (");
				pageSql.append(sql);
				//pageSql.append(") as tmp_tb where ROWNUM<=");
				pageSql.append(") tmp_tb where ROWNUM<=");
				pageSql.append(page.getStartindex()+page.getPagelistcount());
